...
def get_map_path_from_arg(arg: str) -> str:
    ...
    Peut être :
    - Un chemin direct vers un fichier (ex: maps/challenger/01_the_impossible_dream.txt)
    ...
    if "_" in arg:
        ...
        search_pattern = os.path.join(base_dir, "maps", folder, f"{numero}_*.txt")
...
def get_args() -> Dict[str, Any]:
    """
    Parse les arguments de la ligne de commande, attend le chemin vers le fichier de map
    ...
    if len(sys.argv) != 2:
        ...
        print("Usage: python main.py <chemin_vers_map.txt> OU <dossier>_<numero> (ex: challenger_01)")
        sys.exit(1)
    ...
    if not resolved_path or not os.path.exists(resolved_path):
        print(f"Erreur: Impossible de trouver la carte pour l'argument '{input_arg}'.")
        sys.exit(1)
    ...
    # Vérification que le dictionnaire a bien les infos requises (sécurité supplémentaire)
