...
#    Updated: 2026/06/03 15:12:20 by opernod          ###   ########lyon.fr    #
#                                                                              #
# **************************************************************************** #
...
run:
	@printf "$(COLOR_CYAN)Starting simulation with map: $(MAP)$(COLOR_RESET)\n"
	@QT_LOGGING_RULES="Qt3D.Renderer.RHI.Backend=false" \
...
test:
	@printf "$(COLOR_CYAN)🧪 Map robustness test (t_04 to t_38)...$(COLOR_RESET)\n"
	@for map in $$(seq -f "t_%02g" 4 38); do \
		printf "Map $$map: "; \
		QT_LOGGING_RULES="Qt3D.Renderer.RHI.Backend=false" uv run python sources/main.py $$map > /dev/null 2>&1; \
		if [ $$? -eq 0 ]; then \
			printf "$(COLOR_RED)❌ ERROR (The program should not have started)$(COLOR_RESET)\n"; \
			exit 1; \
		else \
			printf "$(COLOR_GREEN)✓ Expected crash OK$(COLOR_RESET)\n"; \
		fi; \
	done
	@printf "$(COLOR_GREEN)✓ Success: All invalid maps were correctly rejected! 🎉$(COLOR_RESET)\n"
